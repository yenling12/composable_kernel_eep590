...
__launch_bounds__(CK_MAX_THREAD_PER_BLOCK, CK_MIN_BLOCK_PER_CU)
  ...
  GridwiseGemm::template Run<HasMainKBlockLoop>(
      ...
      arg_ptr[group_id].p_c_grid_ + c_batch_offset, p_shared, a_element_op,
      b_element_op, acc_element_op, b1_element_op, c_element_op,
      arg_ptr[group_id].a_grid_desc_ak0_m_ak1_,
      arg_ptr[group_id].b_grid_desc_bk0_n_bk1_,
      arg_ptr[group_id].b1_grid_desc_bk0_n_bk1_,
      arg_ptr[group_id].c_grid_desc_mblock_mperblock_nblock_nperblock_,
      arg_ptr[group_id].block_2_ctile_map_, arg_ptr[group_id].c0_matrix_mask_);
#else
...
template <
    ...
    typename CDataType, typename Acc0BiasDataType, typename Acc1BiasDataType,
    typename GemmAccDataType, typename CShuffleDataType,
    typename AElementwiseOperation, typename BElementwiseOperation,
    typename AccElementwiseOperation, typename B1ElementwiseOperation,
    typename CElementwiseOperation, GemmSpecialization GemmSpec,
...
struct DeviceGroupedGemmSoftmaxGemmPermute_Xdl_CShuffle
    : public DeviceGroupedGemmSoftmaxGemmPermute<
          NumDimG, NumDimM, NumDimN, NumDimK, NumDimO, ADataType, BDataType,
          B1DataType, CDataType, Acc0BiasDataType, Acc1BiasDataType,
          AElementwiseOperation, BElementwiseOperation, AccElementwiseOperation,
          B1ElementwiseOperation, CElementwiseOperation, MaskingSpec> {
  ...
  using ProblemDesc = typename DeviceGroupedGemmSoftmaxGemmPermute<
      NumDimG, NumDimM, NumDimN, NumDimK, NumDimO, ADataType, BDataType,
      B1DataType, CDataType, Acc0BiasDataType, Acc1BiasDataType,
      AElementwiseOperation, BElementwiseOperation, AccElementwiseOperation,
      B1ElementwiseOperation, CElementwiseOperation, MaskingSpec>::ProblemDesc;
  ...
  using GridwiseGemm = GridwiseBatchedGemmSoftmaxGemm_Xdl_CShuffle<
      ...
      GemmAccDataType, CShuffleDataType, CDataType, AElementwiseOperation,
      BElementwiseOperation, AccElementwiseOperation, B1ElementwiseOperation,
      CElementwiseOperation, InMemoryDataOperationEnum::Set,
      AGridDesc_AK0_M_AK1, BGridDesc_BK0_N_BK1, B1GridDesc_BK0_N_BK1,
      CGridDesc_M_N, NumGemmKPrefetchStage, BlockSize, MPerBlock, NPerBlock,
      KPerBlock, Gemm1NPerBlock, Gemm1KPerBlock, AK1, BK1, B1K1, MPerXDL,
      NPerXDL, MXdlPerWave, NXdlPerWave, Gemm1NXdlPerWave,
  ...
  struct GroupDeviceArg {
    // lengths for the last dimensions of overall problem for sanity check of
    // vector load/store
    std::vector<index_t> raw_lengths_mz_nz_kz_gemm1nz_;

    // strides for the last dimensions of each tensor for sanity check of vector
    // load/store
  ...
  struct Argument : public BaseArgument {
    Argument(std::vector<const void *> p_a_vec,
             ...
             std::vector<const void *> p_b1_vec, std::vector<void *> p_c_vec,
      ...
      if (!(group_count_ == p_a_vec.size() && group_count_ == p_b_vec.size() &&
            group_count_ == p_b1_vec.size() &&
            group_count_ == p_c_vec.size())) {
      ...
      for (std::size_t i = 0; i < group_count_; i++) {
        ...
        const auto block_2_ctile_map =
            Block2CTileMap(c_grid_desc_m_n, BlockStart);
        ...
        const auto compute_base_ptr_of_batch =
            ComputeBasePtrOfStridedBatch(a_grid_desc_g_m_k, b_grid_desc_g_n_k,
        ...
        // for each group, make sure acc0_biases_gs_ms_ns_lengths.size() ==
        // NumAcc0Bias and so on
        if (!(problem_desc.acc0_biases_gs_ms_ns_lengths.size() == NumAcc0Bias &&
              ...
              problem_desc.acc1_biases_gs_ms_os_strides.size() ==
                  NumAcc1Bias)) {
          throw std::runtime_error(
        ...
        group_kernel_args_.push_back(
            {p_a_grid, p_b_grid, p_b1_grid, p_c_grid, a_grid_desc_ak0_m_ak1,
             b_grid_desc_bk0_n_bk1, b1_grid_desc_bk0_n_bk1,
             c_grid_desc_mblock_mperblock_nblock_nperblock,
             block_2_ctile_map.CalculateGridSize(c_grid_desc_m_n),
             compute_base_ptr_of_batch, c0_matrix_mask, block_2_ctile_map,
             BlockStart, BlockEnd});
        ...
        group_device_args_.push_back(
            {{problem_desc.a_gs_ms_ks_lengths[NumDimG + NumDimM - 1],
             ...
             {problem_desc.b0_gs_ns_ks_strides[NumDimG + NumDimN - 1],
              problem_desc
                  .b0_gs_ns_ks_strides[NumDimG + NumDimN + NumDimK - 1]},
             {problem_desc.b1_gs_os_ns_strides[NumDimG + NumDimO - 1],
              problem_desc
                  .b1_gs_os_ns_strides[NumDimG + NumDimO + NumDimN - 1]},
  ...
  struct Invoker : public BaseInvoker {
    ...
    float Run(const Argument &arg,
      ...
      hipGetErrorString(hipMemcpyWithStream(
          arg.p_workspace_, arg.group_kernel_args_.data(),
          arg.group_kernel_args_.size() * sizeof(GroupKernelArg),
          hipMemcpyHostToDevice, stream_config.stream_id_));

      float ave_time = 0;

      auto launch_kernel = [&](auto has_main_k_block_loop_) {
        const auto kernel = kernel_grouped_gemm_softmax_gemm_xdl_cshuffle_v1<
            GridwiseGemm, GroupKernelArg, AElementwiseOperation,
            BElementwiseOperation, AccElementwiseOperation,
            B1ElementwiseOperation, CElementwiseOperation,
            has_main_k_block_loop_>;

        return launch_and_time_kernel(
            stream_config, kernel, dim3(arg.grid_size_), dim3(BlockSize), 0,
            cast_pointer_to_constant_address_space(arg.p_workspace_),
            arg.group_count_, arg.a_element_op_, arg.b_element_op_,
            arg.acc_element_op_, arg.b1_element_op_, arg.c_element_op_);
      };
  ...
  static bool IsSupportedArgument(const Argument &arg) {
    ...
    for (std::size_t i = 0; i < arg.group_count_; i++) {
      ...
      const index_t b1_gemm1n =
          kernel_arg.b1_grid_desc_bk0_n_bk1_.GetLength(I1);
      ...
      // load when part of vector is out of bounds
      ...
      const auto c_stride_lowest =
          device_arg.c_mz_gemm1nz_strides_[1]; // cshuffle assumes lowest dim in
                                               // Gemm1Ns to be contiguous
  ...
  static auto MakeArgument(
      std::vector<const void *> p_a_vec, std::vector<const void *> p_b_vec,
      std::vector<const void *> p_b1_vec, std::vector<void *> p_c_vec,
      std::vector<std::vector<const void *>> p_acc0_biases_vec,
      std::vector<std::vector<const void *>> p_acc1_biases_vec,
      std::vector<ProblemDesc> problem_desc_vec,
      ...
      B1ElementwiseOperation b1_element_op,
      CElementwiseOperation c_element_op) {
    return Argument{p_a_vec,          p_b_vec,           p_b1_vec,
                    p_c_vec,          p_acc0_biases_vec, p_acc1_biases_vec,
                    problem_desc_vec, a_element_op,      b_element_op,
                    acc_element_op,   b1_element_op,     c_element_op};
  }
  ...
  std::unique_ptr<BaseArgument> MakeArgumentPointer(
    ...
    return std::make_unique<Argument>(
        p_a_vec, p_b_vec, p_b1_vec, p_c_vec, p_acc0_biases_vec,
        p_acc1_biases_vec, problem_desc_vec, a_element_op, b_element_op,
        acc_element_op, b1_element_op, c_element_op);
  }
