...
// Q [G0, G1_Q, M, K] * K [G0, 1, K, N] = P [G0, G1_Q, M, N]
// P [G0, G1_Q, M, N] * V [G0, 1, N, O] = Out [G0, G1_Q, M, O]
...
__launch_bounds__(CK_MAX_THREAD_PER_BLOCK, CK_MIN_BLOCK_PER_CU)
  ...
  std::array<ck::index_t, array_size> a_gs_ms_ks_lengths{G0, G1_Q, M, K};
  std::array<ck::index_t, array_size> a_gs_ms_ks_strides =
      input_permute
          ? std::array<ck::index_t, array_size>{M * G1_Q * K, K, G1_Q * K, 1}
          // A layout [G0, M, G1_Q, K]
          : std::array<ck::index_t, array_size>{G1_Q * M * K, M * K, K,
                                                1}; // A layout [G0, G1_Q, M, K]

  std::array<ck::index_t, array_size> b0_gs_ns_ks_lengths{G0, G1_KV, N, K};
  std::array<ck::index_t, array_size> b0_gs_ns_ks_strides =
      input_permute
          ? std::array<ck::index_t, array_size>{N * G1_KV * K, K, G1_KV * K, 1}
          // B0 layout [G0, N, 1, K]
          : std::array<ck::index_t, array_size>{G1_KV * N * K, N * K, K,
                                                1}; // B0 layout [G0, 1, N, K]

  std::array<ck::index_t, array_size> b1_gs_os_ns_lengths{G0, G1_KV, O, N};
  std::array<ck::index_t, array_size> b1_gs_os_ns_strides =
      input_permute
          ? std::array<ck::index_t, array_size>{N * G1_KV * O, O, 1, G1_KV * O}
          // B1 layout [G0, N, 1, O]
          : std::array<ck::index_t, array_size>{G1_KV * N * O, N * O, 1,
                                                O}; // B1 layout [G0, 1, N, O]

  std::array<ck::index_t, array_size> c_gs_ms_os_lengths{G0, G1_Q, M, O};
  std::array<ck::index_t, array_size> c_gs_ms_os_strides =
      output_permute
          ? std::array<ck::index_t, array_size>{M * G1_Q * O, O, G1_Q * O, 1}
          // C layout [G0, M, G1_Q, O]
          : std::array<ck::index_t, array_size>{G1_Q * M * O, M * O, O,
                                                1}; // C layout [G0, G1_Q, M, O]
  ...
  // fail to reuse DeviceOp::MakeArgument() because of the __device__ function
  // required.
  ...
  const auto a_grid_desc_g_m_k = DeviceOp::Transform::MakeAGridDescriptor_G_M_K(
      a_gs_ms_ks_lengths, a_gs_ms_ks_strides);
  const auto b0_grid_desc_g_l_k =
      DeviceOp::Transform::MakeB0GridDescriptor_G_N_K(b0_gs_ns_ks_lengths,
                                                      b0_gs_ns_ks_strides);
  const auto b1_grid_desc_g_n_l =
      DeviceOp::Transform::MakeB1GridDescriptor_G_N_K(b1_gs_os_ns_lengths,
                                                      b1_gs_os_ns_strides);
  const auto c_grid_desc_g_m_n = DeviceOp::Transform::MakeCGridDescriptor_G_M_N(
      c_gs_ms_os_lengths, c_gs_ms_os_strides);
  const auto compute_base_ptr_of_batch =
      typename DeviceOp::ComputeBasePtrOfStridedBatch{
          a_grid_desc_g_m_k, b0_grid_desc_g_l_k, b1_grid_desc_g_n_l,
          c_grid_desc_g_m_n};
  ...
  const long_index_t b0_batch_offset =
      __builtin_amdgcn_readfirstlane(static_cast<long_index_t>(
          compute_base_ptr_of_batch.GetB0BasePtr(g_idx * G1_KV / G1_Q)));
  const long_index_t b1_batch_offset =
      __builtin_amdgcn_readfirstlane(static_cast<long_index_t>(
          compute_base_ptr_of_batch.GetB1BasePtr(g_idx * G1_KV / G1_Q)));
...
struct DeviceGroupedQueryAttentionForward_Wmma
  ...
  struct RawArg : public BaseArgument {
    RawArg(const ADataType *p_a_grid, const B0DataType *p_b0_grid,
           ...
           index_t G1_KV, float alpha, bool input_permute, bool output_permute)
  ...
  static auto MakeArgument(const void *p_a, const void *p_b0, const void *p_b1,
                           ...
                           index_t O, index_t G0, index_t G1_Q, index_t G1_KV,
  ...
  static bool IsSupportedArgument(const RawArg &arg) {
    ...
    ck::index_t G1_Q = arg.G1_Q_;
    ck::index_t G1_KV = arg.G1_KV_;
    ck::index_t M = arg.M_;
    ck::index_t N = arg.N_;
    ck::index_t K = arg.K_;
    ck::index_t O = arg.O_;
    ...
    std::array<ck::index_t, array_size> a_gs_ms_ks_strides =
        input_permute
            ...
            : std::array<ck::index_t, array_size>{
                  G1_Q * M * K, M * K, K, 1}; // A layout [G0, G1_Q, M, K]

    std::array<ck::index_t, array_size> b0_gs_ns_ks_lengths{G0, G1_Q, N, K};
    std::array<ck::index_t, array_size> b0_gs_ns_ks_strides =
        input_permute
            ? std::array<ck::index_t, array_size>{N * G1_Q * K, K, G1_Q * K, 1}
            // B0 layout [G0, N, G1_Q, K]
            : std::array<ck::index_t, array_size>{
                  G1_Q * N * K, N * K, K, 1}; // B0 layout [G0, G1_Q, N, K]

    std::array<ck::index_t, array_size> b1_gs_os_ns_lengths{G0, G1_Q, O, N};
    std::array<ck::index_t, array_size> b1_gs_os_ns_strides =
        input_permute
            ? std::array<ck::index_t, array_size>{N * G1_Q * O, O, 1, G1_Q * O}
            // B1 layout [G0, N, G1_Q, O]
            : std::array<ck::index_t, array_size>{
                  G1_Q * N * O, N * O, 1, O}; // B1 layout [G0, G1_Q, N, O]
    ...
    std::array<ck::index_t, array_size> c_gs_ms_os_strides =
        output_permute
            ...
            // C layout [G0, M, G1_Q, O]
            : std::array<ck::index_t, array_size>{
                  G1_Q * M * O, M * O, O, 1}; // C layout [G0, G1_Q, M, O]
  ...
  struct Invoker : public BaseInvoker {
    ...
    float Run(const Argument &arg,
      ...
      const index_t grid_size = arg.G0_ * arg.G1_Q_ * M0 * N0;
      const auto K = arg.K_;
      // printf("HasKBlockLoop: %d\n",
      // GridwiseOp::CalculateHasMainKBlockLoop(K));
      auto launch_kernel = [&](auto has_main_k_block_loop) {
        const auto kernel = kernel_grouped_query_attention_wmma<
        ...
        return launch_and_time_kernel(
            ...
            arg.M_, arg.N_, arg.K_, arg.O_, arg.G0_, arg.G1_Q_, arg.G1_KV_,
            arg.alpha_, arg.input_permute_, arg.output_permute_);
      };
