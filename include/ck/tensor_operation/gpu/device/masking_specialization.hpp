...
inline std::string
getMaskingSpecializationString(const MaskingSpecialization &s) {
  switch (s) {
  case MaskingSpecialization::MaskDisabled:
    return "MaskDisabled";
  case MaskingSpecialization::MaskUpperTriangleFromTopLeft:
    return "MaskUpperTriangleFromTopLeft";
  case MaskingSpecialization::MaskUpperTriangleFromBottomRight:
    return "MaskUpperTriangleFromBottomRight";
  default:
    return "Unrecognized specialization!";
  }
}

struct MaskDisabledPredicate {
  __host__ __device__ constexpr bool operator()(index_t /*m*/,
                                                index_t /*n*/) const {
    return false;
  };
...
struct MaskUpperTriangleFromTopLeftPredicate {
  ...
  __host__ __device__ constexpr bool IsTileSkippable(index_t m, index_t n,
                                                     index_t m_tile,
                                                     index_t /*n_tile*/) const {
...
struct MaskUpperTriangleFromBottomRightPredicate {
  ...
  IsTileSkippable(index_t m_tile_orig, index_t n_tile_orig, index_t m_tile_size,
...
template <typename MaskOutPredicate> struct C0MatrixMask_impl {
  ...
  __host__ __device__ constexpr bool
  IsNOutOfBound(/*index_t m, */ index_t n) const {
    return n >= NRaw_;
  }

  __host__ __device__ constexpr bool IsMaskedElement(index_t m,
                                                     index_t n) const {
