    ...
    kernel_gemm_xdl_cshuffle_v3(typename GridwiseGemm::Argument karg)
{
    ...
    GridwiseGemm::template Run<HasMainKBlockLoop, CGlobalMemoryDataOperation, TailNum>(
        ...
        karg);
#else
    GridwiseGemm::template RunKBatch<HasMainKBlockLoop, CGlobalMemoryDataOperation, TailNum>(
        karg.p_a_grid + splitk_batch_offset.a_k_split_offset,
        karg.p_b_grid + splitk_batch_offset.b_k_split_offset,
        karg.p_c_grid,
        p_shared,
...
struct GridwiseGemm_xdl_cshuffle_v3
{
    ...
    __host__ __device__ static auto MakeCGridDescriptor_M_N_KBatch(
        index_t M, index_t MPad, index_t N, index_t NPad, index_t StrideC, index_t KBatch)
    {
        const auto c_grid_desc_mraw_nraw_kbatch = [&]() {
            if constexpr(is_same<tensor_layout::gemm::RowMajor, CLayout>::value)
            {
                return make_naive_tensor_descriptor(make_tuple(M, N, KBatch),
                                                    make_tuple(StrideC, KBatch, I1));
            }
            else if constexpr(is_same<tensor_layout::gemm::ColumnMajor, CLayout>::value)
            {
                return make_naive_tensor_descriptor(make_tuple(M, N, KBatch),
                                                    make_tuple(KBatch, StrideC, I1));
            }
        }();
        ...
        {
            // not pad M or N
            return c_grid_desc_mraw_nraw_kbatch;
        }
    }
    ...
    template <typename CGridKBatchDesc>
    __device__ static constexpr auto MakeCGridDescriptor_MBlock_MPerBlock_NBlock_NPerBlock_KBatch(
        const CGridKBatchDesc& c_grid_desc_m_n_kbatch,
        index_t MBlock,
        index_t NBlock,
        const index_t k_batch)
    {
        const auto c_grid_desc_mblock_mperblock_nblock_nperblock_kbatch =
            transform_tensor_descriptor(
                c_grid_desc_m_n_kbatch,
                make_tuple(make_unmerge_transform(make_tuple(MBlock, Number<MPerBlock>{})),
                           make_unmerge_transform(make_tuple(NBlock, Number<NPerBlock>{})),
                           make_pass_through_transform(k_batch)),
                make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}),
                make_tuple(Sequence<0, 1>{}, Sequence<2, 3>{}, Sequence<4>{}));

        return c_grid_desc_mblock_mperblock_nblock_nperblock_kbatch;
    }
    ...
    template <bool HasMainKBlockLoop,
              InMemoryDataOperationEnum CGlobalMemoryDataOperation,
              TailNumber TailNum = TailNumber::Odd>
    __device__ static void RunKBatch(const FloatA* p_a_grid,
                                     const FloatB* p_b_grid,
                                     FloatC* p_c_kbatch_grid,
                                     void* p_shared,
                                     const Problem& problem)
    {
        const auto a_grid_desc_ak0_m_ak1 = MakeAGridDescriptor_AK0_M_AK1(
