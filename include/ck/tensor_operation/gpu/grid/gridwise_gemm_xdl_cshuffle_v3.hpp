...
struct GridwiseGemm_xdl_cshuffle_v3
{
    ...
    __device__ static constexpr auto GetBBlockDescriptor_BK0PerBlock_NPerBlock_BK1()
    {
        ...
        else if constexpr(is_same<tensor_layout::gemm::ColumnMajor, BLayout>::value)
        {
            ...
            constexpr auto b_lds_block_desc_bk0_n_bk1 = transform_tensor_descriptor(
                b_lds_block_desc,
                make_tuple(make_xor_transform(make_tuple(Number<NPerBlock>{},
                                                         Number<BK0Number>{})),
                           make_pass_through_transform(BK1Number)),
                make_tuple(Sequence<1, 0>{}, Sequence<2>{}),
                make_tuple(Sequence<1, 0>{}, Sequence<2>{}));
        ...
        else // RowMajor B
        {
            ...
            constexpr auto N0 = BBlockTransferThreadClusterLengths_BK0_N_BK1{}.At(I1);
            constexpr auto N1 = NPerBlock / N0;
            constexpr auto BK0PerThread =
                BK0Number / BBlockTransferThreadClusterLengths_BK0_N_BK1{}.At(I0);
            constexpr auto N0PerLdsLayer = 32 * 4 / (N1 * BK1Number * sizeof(FloatB)) <= 1
                                               ? 1
                                               : 32 * 4 / (N1 * BK1Number * sizeof(FloatB));

            constexpr auto b_lds_block_desc = make_naive_tensor_descriptor_packed(
                make_tuple(Number<BK0Number / BK0PerThread * N0 / N0PerLdsLayer>{},
                           Number<BK0PerThread>{},
                           Number<N0PerLdsLayer>{},
                           Number<N1>{},
                           BK1Number));
            ...
            constexpr auto b_lds_block_desc_permuted = transform_tensor_descriptor(
                ...
                make_tuple(
                    make_xor_transform(make_tuple(
                        Number<BK0Number / BK0PerThread * N0 / N0PerLdsLayer>{}, Number<N1>{})),
            ...
            constexpr auto b_lds_block_desc_unmerged = transform_tensor_descriptor(
                b_lds_block_desc_permuted,
                make_tuple(make_unmerge_transform(make_tuple(Number<BK0Number / BK0PerThread>{},
                                                             Number<N0 / N0PerLdsLayer>{})),
                           make_pass_through_transform(Number<BK0PerThread>{}),
                           make_pass_through_transform(Number<N0PerLdsLayer>{}),
                           make_pass_through_transform(Number<N1>{}),
                           make_pass_through_transform(BK1Number)),
            ...
            constexpr auto b_lds_block_desc_bk0_n_bk1 = transform_tensor_descriptor(
                ...
                make_tuple(
                    ...
                    make_merge_transform_v3_division_mod(make_tuple(
                        Number<N0 / N0PerLdsLayer>{}, Number<N0PerLdsLayer>{}, Number<N1>{})),
                    make_pass_through_transform(BK1Number)),
                make_tuple(Sequence<0, 2>{}, Sequence<1, 3, 4>{}, Sequence<5>{}),
                make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}));

            // Contiguous NPerXdl thread ds_read cannot use all lds banks
            if constexpr(BK1Number * NPerXdl * sizeof(FloatB) < 128)
            {
                constexpr auto WaveSize        = 64;
                constexpr auto BK0PerThreadMMA = BK0Number / (WaveSize / NPerXdl);

                constexpr auto b_lds_block_desc_unmerged_2 = transform_tensor_descriptor(
                    b_lds_block_desc_bk0_n_bk1,
                    make_tuple(
                        make_unmerge_transform(
                            make_tuple(Number<WaveSize / NPerXdl>{}, Number<BK0PerThreadMMA>{})),
                        make_unmerge_transform(make_tuple(Number<N0 / N0PerLdsLayer>{},
                                                          Number<N0PerLdsLayer * N1 / NPerXdl>{},
                                                          Number<NPerXdl>{})),
                        make_pass_through_transform(BK1Number)),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}),
                    make_tuple(Sequence<0, 1>{}, Sequence<2, 3, 4>{}, Sequence<5>{}));

                constexpr auto b_lds_block_desc_permuted_2 = transform_tensor_descriptor(
                    b_lds_block_desc_unmerged_2,
                    make_tuple(
                        make_xor_transform(make_tuple(Number<WaveSize / NPerXdl>{},
                                                      Number<N0PerLdsLayer * N1 / NPerXdl>{})),
                        make_pass_through_transform(Number<BK0PerThreadMMA>{}),
                        make_pass_through_transform(Number<N0 / N0PerLdsLayer>{}),
                        make_pass_through_transform(Number<NPerXdl>{}),
                        make_pass_through_transform(BK1Number)),
                ...
                constexpr auto b_lds_block_desc_merged_2 = transform_tensor_descriptor(
                    b_lds_block_desc_permuted_2,
                    make_tuple(make_merge_transform_v3_division_mod(make_tuple(
                                   Number<WaveSize / NPerXdl>{}, Number<BK0PerThreadMMA>{})),
                               make_merge_transform_v3_division_mod(
                                   make_tuple(Number<N0 / N0PerLdsLayer>{},
                                              Number<N0PerLdsLayer * N1 / NPerXdl>{},
                                              Number<NPerXdl>{})),
                               make_pass_through_transform(BK1Number)),
                    make_tuple(Sequence<0, 1>{}, Sequence<2, 3, 4>{}, Sequence<5>{}),
                ...
                return b_lds_block_desc_merged_2;
            }
            else { return b_lds_block_desc_bk0_n_bk1; }
#elif 1
            // N0PerLdsLayer * N1 as logical Bank
            constexpr auto N0 = BBlockTransferThreadClusterLengths_BK0_N_BK1{}.At(I1);
            constexpr auto N1 = NPerBlock / N0;

            if constexpr((BK1Number * N0 * sizeof(FloatB) >128) || (BK1Number * NPerXdl * sizeof(FloatB) <128)){
                constexpr auto KThreadWrite = BBlockTransferThreadClusterLengths_BK0_N_BK1{}.At(I0); // 16
                constexpr auto K0PerThreadWrite = BK0Number/KThreadWrite; // 2
                constexpr auto KThreadRead = 64/NPerXdl; // 4
                constexpr auto K0PerThreadRead = BK0Number/KThreadRead; // 8
                
                constexpr auto kfold = (BK1Number * N0 * sizeof(FloatB) > 128)? 1: 128/(BK1Number * N0 * sizeof(FloatB)); // 4
                constexpr auto KThreadReadPerm = (kfold*K0PerThreadWrite/K0PerThreadRead)> 1 ? KThreadRead/ (kfold*K0PerThreadWrite/K0PerThreadRead):KThreadRead;

                constexpr auto npair = (BK1Number * NPerXdl * sizeof(FloatB) > 128)? 1: 128/(BK1Number * NPerXdl * sizeof(FloatB));
                constexpr auto b_lds_block_desc = make_naive_tensor_descriptor_packed(
                ...
                constexpr auto b_lds_block_desc_permuted = transform_tensor_descriptor(
                    ...
                    make_tuple(
                        ...
                        make_xor_transform(make_tuple(Number<KThreadReadPerm*N1>{}, Number<kfold*N0/npair>{})),
                ...
                constexpr auto b_lds_block_desc_unmerged = transform_tensor_descriptor(
                    ...
                    make_tuple(Sequence<1>{}, Sequence<2>{}, Sequence<0, 3>{}, Sequence<4, 5>{}, Sequence<6>{}, Sequence<7>{}));

                constexpr auto b_lds_block_desc_bk0_n_bk1 = transform_tensor_descriptor(
                    b_lds_block_desc_unmerged,
                    make_tuple(
                        make_merge_transform_v3_division_mod(make_tuple(Number<KThreadReadPerm>{}, Number<KThreadWrite/kfold/KThreadReadPerm>{}, Number<kfold>{}, Number<K0PerThreadWrite>{})),
                        make_merge_transform_v3_division_mod(make_tuple(Number<N0/npair>{}, Number<npair>{}, Number<N1>{})),
                        make_pass_through_transform(BK1Number)),
            ...
            else if constexpr(BK1Number * NPerXdl * sizeof(FloatB) >= 128){
                // No conflict concern cross
                constexpr auto b_lds_block_desc = make_naive_tensor_descriptor(
                    make_tuple(BK0Number,
                               Number<N1>{},
                               Number<N0>{},
                               BK1Number),
                    make_tuple(Number<NPerBlock*BK1Number>{},
                               BK1Number,
                               Number<N1*BK1Number>{},
                               I1));
                ...
                constexpr auto b_lds_block_desc_permuted = transform_tensor_descriptor(
                    b_lds_block_desc,
                    make_tuple(
                        make_xor_transform(make_tuple(Number<N1>{}, Number<N0>{})),
                ...
                constexpr auto b_lds_block_desc_bk0_n_bk1 = transform_tensor_descriptor(
                    ...
                    make_tuple(
                        make_pass_through_transform(BK0Number),
                        make_merge_transform_v3_division_mod(make_tuple(Number<N1>{}, Number<N0>{})),
                        make_pass_through_transform(BK1Number)),
                    make_tuple(Sequence<0>{}, Sequence<1, 2>{}, Sequence<3>{}),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}));
                ...
                // NPerXdl=16, KPerXdl=4
                constexpr auto N2 = 128 / (BK1Number * NPerXdl * sizeof(FloatB));
                constexpr auto K2 = 4;

                constexpr auto b_lds_block_desc = make_naive_tensor_descriptor_packed(
                    make_tuple(Number<BK0Number/K2>{},
                               Number<K2*N1>{},
                               Number<N0/N2>{},
                               Number<N2>{},
                               BK1Number));
                ...
                constexpr auto b_lds_block_desc_permuted = transform_tensor_descriptor(
                    b_lds_block_desc,
                    make_tuple(
                        make_xor_transform(make_tuple(Number<K2*N1>{}, Number<N0/N2>{})),
                        make_pass_through_transform(Number<BK0Number/K2>{}),
                        make_pass_through_transform(Number<N2>{}),
                        make_pass_through_transform(BK1Number)),
                    make_tuple(Sequence<1, 2>{}, Sequence<0>{}, Sequence<3>{}, Sequence<4>{}),
                    make_tuple(Sequence<1, 2>{}, Sequence<0>{}, Sequence<3>{}, Sequence<4>{}));
                ...
                constexpr auto b_lds_block_desc_unmerged = transform_tensor_descriptor(
                    ...
                    make_tuple(
                        make_pass_through_transform(Number<BK0Number/K2>{}),
                        make_unmerge_transform(make_tuple(Number<K2>{}, Number<N1>{})),
                        make_pass_through_transform(Number<N0/N2>{}),
                        make_pass_through_transform(Number<N2>{}),
                        make_pass_through_transform(BK1Number)),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}, Sequence<3>{}, Sequence<4>{}),
                    make_tuple(Sequence<1>{}, Sequence<0, 2>{}, Sequence<3>{}, Sequence<4>{}, Sequence<5>{}));

                constexpr auto b_lds_block_desc_bk0_n_bk1 = transform_tensor_descriptor(
                    b_lds_block_desc_unmerged,
                    make_tuple(
                        make_merge_transform_v3_division_mod(make_tuple(Number<K2>{}, Number<BK0Number/K2>{})),
                        make_merge_transform_v3_division_mod(make_tuple(Number<N0/N2>{}, Number<N2>{}, Number<N1>{})),
                        make_pass_through_transform(BK1Number)),
                    make_tuple(Sequence<0, 1>{}, Sequence<3, 4, 2>{}, Sequence<5>{}),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}));
