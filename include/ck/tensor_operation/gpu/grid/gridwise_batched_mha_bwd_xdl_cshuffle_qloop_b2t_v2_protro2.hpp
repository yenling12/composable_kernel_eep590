...
struct GridwiseBatchedMultiheadAttentionBackward_Qloop_Xdl_CShuffle_V2
{
    ...
    {
        constexpr auto K3 = BK1;
        constexpr auto K2 = mfma.num_input_blks;
        constexpr auto K1 = KPerBlock / K2 / K3;
        constexpr auto K0 = Gemm1NPerBlock / KPerBlock;
        constexpr auto N1 = NXdlPerWave;

        // V matrix in Vgpr, dst of threadwise copy
        return make_naive_tensor_descriptor_packed(
            make_tuple(Number<K0>{}, Number<K1>{}, I1, I1, Number<N1>{}, I1, I1, Number<K3>{}));
    }
    ...
    struct GemmBlockwiseCopy
    {
        ...
        {
            const auto V_K0 = v_grid_desc_o0_n_o1.GetLength(I0);
            const auto N    = v_grid_desc_o0_n_o1.GetLength(I1);
            const auto V_K1 = v_grid_desc_o0_n_o1.GetLength(I2);
            // const auto K    = V_K0 * V_K0;
            ...
            // constexpr auto K3 = V_K0;
            // constexpr auto K2 = mfma.num_input_blks;
            // constexpr auto K1 = KPerBlock / K2 / K3;
            // const auto K0     = K / KPerBlock;

            constexpr auto N3 = NPerXdl;
            ...
            const auto v_grid_desc_n_k = transform_tensor_descriptor(
                v_grid_desc_o0_n_o1,
                make_tuple(make_pass_through_transform(N),
                           make_merge_transform_v3_division_mod(make_tuple(V_K0, V_K1))),
                make_tuple(Sequence<1>{}, Sequence<0, 2>{}),
                make_tuple(Sequence<0>{}, Sequence<1>{}));

            return transform_tensor_descriptor(
                v_grid_desc_n_k,
                make_tuple(make_unmerge_transform(make_tuple(N0, N1, N2, N3)),
                           make_unmerge_transform(make_tuple(K0, K1, K2, K3))),
                make_tuple(Sequence<0>{}, Sequence<1>{}),
    ...
    struct Gemm0
    {
        ...
        static constexpr auto b_k3 = BK1;
        static constexpr auto b_k2 = mfma.num_input_blks;
        static constexpr auto b_k1 = KPerBlock / b_k2 / b_k3;
        static constexpr auto b_n1 = NXdlPerWave;
        ...
        static constexpr auto b_thread_desc_k0_n_k1 = make_naive_tensor_descriptor_packed(
            make_tuple(Number<b_k1>{}, Number<b_n1>{}, Number<b_k3>{}));
        ...
        using BBlockwiseCopy = ThreadwiseTensorSliceTransfer_StaticToStatic<
            GemmDataType,
            GemmDataType,
            decltype(b_src_thread_desc_k0_n_k1),
            decltype(b_thread_desc_k0_n_k1),
            tensor_operation::element_wise::PassThrough,
            decltype(b_thread_desc_k0_n_k1.GetLengths()),
            Sequence<1, 0, 2>,
            2,
            b_k3>;
        ...
        static constexpr auto b_block_slice_copy_step = make_tuple(Number<b_k1>{}, I0, I0);
    };
                               ...
                               const index_t block_idx_n)
    {
        ...
        auto gemm0_b_thread_buf = make_static_buffer<AddressSpaceEnum::Vgpr, GemmDataType>(
            Gemm0::b_thread_desc_k0_n_k1.GetElementSpaceSize());
        ...
        auto pgrad_gemm_tile_v_blockwise_copy =
            typename Gemm0::BBlockwiseCopy{tensor_operation::element_wise::PassThrough{}};
        ...
        //     KPerBlock);
        constexpr index_t num_o_block_main_loop = Gemm1NPerBlock / KPerBlock;
        ...
        do
        {
            ...
            // S = Q * K^T
            {
                ...
                if constexpr(HasMainKBlockLoop)
                {
                    ...
                    do
                    {
                        ...
                        ++i;
                    } while(i < (num_k_block_main_loop - 1));
                }
                // tail
                {
                    block_sync_lds();
            ...
            // assume size K == size O so HasMainKBlockLoop is the same
            {
                ...
                // main body
                if constexpr(num_o_block_main_loop > 1)
                {
                    static_for<0, num_o_block_main_loop - 1, 1>{}([&](auto i) {
                        pgrad_gemm_tile_v_blockwise_copy.Run(Gemm0::b_src_thread_desc_k0_n_k1,
                                                             Gemm0::b_block_slice_copy_step * i,
                                                             v_thread_buf,
                                                             Gemm0::b_thread_desc_k0_n_k1,
                                                             make_tuple(I0, I0, I0),
                                                             gemm0_b_thread_buf);
                        ...
                        pgrad_blockwise_gemm.Run(
                            gemm0_a_block_buf, gemm0_b_thread_buf, pgrad_thread_buf);
                ...
                // tail
                {
                    pgrad_gemm_tile_v_blockwise_copy.Run(
                        Gemm0::b_src_thread_desc_k0_n_k1,
                        Gemm0::b_block_slice_copy_step * Number<num_o_block_main_loop - 1>{},
                        v_thread_buf,
                        Gemm0::b_thread_desc_k0_n_k1,
                        make_tuple(I0, I0, I0),
                        gemm0_b_thread_buf);
