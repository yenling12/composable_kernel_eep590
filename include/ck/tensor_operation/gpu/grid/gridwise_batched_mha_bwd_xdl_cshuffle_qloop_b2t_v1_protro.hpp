...
struct GridwiseBatchedMultiheadAttentionBackward_Qloop_Xdl_CShuffle_V1
{
    ...
    struct Gemm1
    {
        ...
        {
            return MakeGemmMmaTileDescriptor_MN0_MN1_MN2_K<Gemm1NXdlPerWave, Gemm0MWaves, NPerXdl>(
                BBlockDesc_BK0_N_BK1{});
        }
        ...
        static constexpr index_t B_N0 = AK0;
        static constexpr index_t B_K  = MPerBlock;
        static constexpr index_t B_N1 = AK1;
        static constexpr index_t B_N  = KPerBlock;
        static constexpr index_t B_K1 = 1;
        static constexpr index_t B_K0 = B_K / B_K1;

        __host__ __device__ static constexpr auto MakeBBlockDesc_K0_N_K1()
        {
            constexpr auto b_block_desc_n_k = transform_tensor_descriptor(
                b_block_desc_bn0_k_bn1,
                make_tuple(make_merge_transform_v3_division_mod(
                               make_tuple(Number<B_N0>{}, Number<B_N1>{})),
                           make_pass_through_transform(Number<B_K>{})),
                make_tuple(Sequence<0, 2>{}, Sequence<1>{}),
                make_tuple(Sequence<0>{}, Sequence<1>{}));

            return transform_tensor_descriptor(
                b_block_desc_n_k,
                make_tuple(make_unmerge_transform(make_tuple(Number<B_K0>{}, Number<B_K1>{})),
                           make_pass_through_transform(Number<B_N>{})),
                make_tuple(Sequence<1>{}, Sequence<0>{}),
                make_tuple(Sequence<0, 2>{}, Sequence<1>{}));
        }

        static constexpr auto b_block_desc_k0_n_k1 = MakeBBlockDesc_K0_N_K1();

        static constexpr auto b_block_slice_copy_step = make_multi_index(0, 0, 0, Gemm1KPerBlock);
        static constexpr auto b_block_reset_copy_step = make_multi_index(0, 0, 0, -MPerBlock);
        ...
        using BlockwiseGemm = BlockwiseGemmXdlops_v2<
            ...
            decltype(MakeGemm1BMmaTileDescriptor_N0_N1_N2_K(b_block_desc_k0_n_k1)),
            NPerBlock,
            Gemm1NPerBlock,
            Gemm1KPerBlock,
            MPerXdl,
                               ...
                               const index_t block_idx_n)
    {
        ...
        // dV: blockwise gemm
        auto vgrad_blockwise_gemm = typename Gemm1::BlockwiseGemm{make_tuple(0, 0, 0, 0)};
        ...
        // dK: blockwise gemm
        auto kgrad_blockwise_gemm = typename Gemm1::BlockwiseGemm{make_tuple(0, 0, 0, 0)};
        ...
        do
        {
            ...
            // dV = P_drop^T * dY
            {
                ...
                static_for<0, num_gemm1_k_block_inner_loop, 1>{}([&](auto i) {
                    ...
                    vgrad_blockwise_gemm.Run(gemm1_a_thread_buf, ygrad_block_buf, vgrad_thread_buf);

                    vgrad_blockwise_gemm.MoveBBlockSrcSliceWindow(Gemm1::b_block_slice_copy_step);
            ...
            // dK = scalar * dS^T * Q
            {
                ...
                static_for<0, num_gemm1_k_block_inner_loop, 1>{}([&](auto i) {
                    ...
                    kgrad_blockwise_gemm.Run(gemm1_a_thread_buf, q_block_buf, kgrad_thread_buf);

                    kgrad_blockwise_gemm.MoveBBlockSrcSliceWindow(Gemm1::b_block_slice_copy_step);
            ...
            vgrad_blockwise_gemm.MoveBBlockSrcSliceWindow(Gemm1::b_block_reset_copy_step);
            kgrad_blockwise_gemm.MoveBBlockSrcSliceWindow(Gemm1::b_block_reset_copy_step);
