...
/**
 * @brief      Batched - grouped 2D space-filling curve, construct 3D grid block mapping
 *
 * @paragraph  Description
 *             This Block-to-C-tile-map creates a 3D grid (n_blocks, m_blocks, z_blocks) of thread
 *             blocks. The first 2D are regular 2D tiles created by division of output GEMM
 ...
 */
template <index_t GroupNum, index_t MPerBlock, index_t NPerBlock>
struct BlockToCTileMap_Grouped_3DGrid_KSplit
{

    __host__ __device__ BlockToCTileMap_Grouped_3DGrid_KSplit() = default;
    ...
    __device__ constexpr auto CalculateBottomIndex(index_t M, index_t N, index_t M01_) const
    {
        const auto M0 = math::integer_divide_ceil(M, MPerBlock);
        const auto N0 = math::integer_divide_ceil(N, NPerBlock);

        const auto block_1d_id = blockIdx.y * N0 + blockIdx.x; 
        const auto group_size  = math::integer_divide_ceil(M0 * N0, GroupNum);
        auto group_id          = block_1d_id % GroupNum;
        auto remap_block_1d_id = group_id * group_size + block_1d_id / GroupNum;
